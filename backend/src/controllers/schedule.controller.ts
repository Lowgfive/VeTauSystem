...
export class ScheduleService {
  // core business logic; callers should supply a plain, trimmed id
  static async generateSchedules(
    trainId: string,
    maxDays: number = 30
  ) {
    // defensive normalization in case somebody still passes the request
    // object by mistake (was the original issue).
  ...
  // express-style wrapper that pulls parameters from the body and sends a response
  static async generateSchedulesHandler(req: Request, res: Response, next: NextFunction) {
    try {
      const { trainId, maxDays = 30 } = req.body;

      if (!mongoose.Types.ObjectId.isValid(trainId)) {
        return res.status(400).json({ success: false, message: "Invalid train id" });
      }

      const result = await ScheduleService.generateSchedules(trainId, maxDays);
